...
export default function Portfolio() {
  ...
  const [activeFile, setActiveFile] = useState<EditorFile>('Home.tsx')
  const [openTabs, setOpenTabs] = useState<EditorFile[]>(defaultTabs)
  ...
  const openFile = (file: EditorFile) => {
    setActiveFile(file)
    setOpenTabs((current) => (current.includes(file) ? current : [...current, file]))
    setMobileNav(false)
  }
  ...
  const activateTab = (file: EditorFile) => {
    const sectionByFile: Partial<Record<EditorFile, Section>> = {
      'Home.tsx': 'home',
      'About.md': 'about',
    ...
    section ? navigate(section) : openFile(file)
  }
  ...
  return (
    <div ref={root} className="min-h-screen bg-editor font-mono text-sm text-zinc-300">
      ...
      <Explorer activeFile={activeFile} onNavigate={navigate} onOpenFile={openFile} />
      <MobileExplorer open={mobileNav} onClose={() => setMobileNav(false)} onNavigate={navigate} />
